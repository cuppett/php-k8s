...
CRUD operations (Create, Read, Update, Delete) are available once you have [configured authentication](/guide/getting-started/authentication) to your Kubernetes cluster.
...
The `update()` method uses Kubernetes REPLACE operation, which replaces the entire resource. For partial updates, use [JSON Patch](/guide/usage/patching) instead.
:::
...
- [Patching](/guide/usage/patching) - Perform partial updates with JSON Patch
- [Watching Resources](/guide/usage/watching-resources) - Monitor changes in real-time
