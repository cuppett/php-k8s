...
Brief description of the VolumeAttributesClass resource and its purpose in Kubernetes.
...
$resource = K8s::volumeattributesclass($cluster)
    ->setName('my-volumeattributesclass')
    ->setLabels(['app' => 'myapp'])
...
```php
$resource = $cluster->getVolumeAttributesClassByName('my-volumeattributesclass');
```
...
$resource = K8s::volumeattributesclass($cluster)
    ->setName('example-volumeattributesclass')
    ->create();
