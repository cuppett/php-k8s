...
- **Start cluster:** Minikube is the reference. Example: `minikube start --kubernetes-version=v1.37.0`.
...
- **Pull request process:**
  1. Create feature branch from current working branch
  2. Implement changes following existing patterns
  3. Run full test suite locally: `vendor/bin/phpunit`
  4. Run integration tests with minikube: `CI=true vendor/bin/phpunit`
...
- CI matrix runs PHP 8.3-8.5 across Kubernetes v1.36.4, v1.37.0 and Laravel 12/13 with both `prefer-lowest` and `prefer-stable`.
